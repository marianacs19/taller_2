...
library(pacman)
p_load(tidyverse, caret, glmnet, readxl, writexl, leaps)
...
predictSample <- test_def %>%
  mutate(
    pobre_lab = predict(en_model, newdata = test_def, type = "raw"),
    pobre_lab = ifelse(pobre_lab == "No_pobre", 0, 1)
  ) %>%
  select(id, pobre_lab)

predictSample <- test_def %>%
  mutate(
    pobre_lab = predict(model_en_f2, newdata = test_def, type = "raw"),
